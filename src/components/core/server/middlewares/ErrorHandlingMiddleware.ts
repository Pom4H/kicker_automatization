...
export class ErrorHandlingMiddleware
  ...
  public error(
    ...
    const coreHttpError =
      extractedError instanceof HttpError
        ? extractedError
  ...
  protected createCoreHttpError(error: Error): HttpError | null {
    ...
    switch (code) {
      ...
      case HttpCode.EntityTooLarge:
        ...
        if (
          undefined !== bodyParserError.limit &&
          undefined !== bodyParserError.length
        ) {
          result = new NotFoundError(
            `${error.message} (request ${bodyParserError.length}, limit ${
              bodyParserError.limit
            })`
          );
        } else {
          result = new NotFoundError(error.message);
